    ...
    if (finalScore <= 0) {
        ...
        form.onsubmit = async function (e) {
            ...
            if (!leaderboardRegex.test(leaderboardName)) {
                alert("Leaderboard Name contains invalid characters. Please use only letters, numbers, spaces, hyphens (-), or underscores (_).");
                return;
            }
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener("keydown", (e) => {
        ...
        if (e.code === "Space" || e.keyCode === 32) {
            // If the target is NOT an input or textarea, prevent default behavior
...
splashImage.src = 'assets/snek_hunter.png';
splashImage.onload = () => {
    splashScreen.innerHTML = ''; // Clear any previous content
    splashScreen.appendChild(splashImage);

    // Create and append the "Press Space to Start" text
    const startText = document.createElement('div');
    startText.textContent = 'PRESS SPACE TO START';
    startText.className = 'start-text';
    splashScreen.appendChild(startText);
};